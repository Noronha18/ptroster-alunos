...
function StatCard({ label, value, sub, color = 'text-gray-900' }) {
  return (
    <div className="bg-white rounded-2xl p-4 border border-ios-border flex-1">
  ...
  return (
    <div className="min-h-screen bg-ios-background px-4 pt-14 pb-6">
      ...
      <div className="flex justify-between items-start mb-6">
        ...
        <button
          onClick={logout}
          className="p-3 bg-white border border-ios-border rounded-2xl text-ios-text-secondary active:bg-ios-gray transition-colors shadow-sm"
          aria-label="Sair"
        >
      ...
      <div className={`rounded-2xl p-4 mb-5 flex items-center gap-3 ${sessaoHoje ? 'bg-primary/10 border border-primary/20' : 'bg-white border border-ios-border'}`}>
        <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${sessaoHoje ? 'bg-primary' : 'bg-ios-gray'}`}>
          {sessaoHoje ? (
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" className="w-5 h-5">
              <path fillRule="evenodd" d="M19.916 4.626a.75.75 0 01.208 1.04l-9 13.5a.75.75 0 01-1.154.114l-6-6a.75.75 0 011.06-1.06l5.353 5.353 8.493-12.739a.75.75 0 011.04-.208z" clipRule="evenodd" />
            </svg>
          ) : (
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="#8E8E93" className="w-5 h-5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          )}
        </div>
        <div className="flex-1">
          <p className={`font-black text-sm ${sessaoHoje ? 'text-primary' : 'text-gray-900'}`}>
        ...
        {!sessaoHoje && (
          <Link to="/checkin" className="shrink-0 bg-primary text-white text-xs font-black px-3 py-2 rounded-xl">
            Ir
          </Link>
        )}
      </div>
      ...
      <div className="flex gap-3 mb-5">
        ...
        <StatCard
          ...
          sub="aulas restantes"
          color={(aluno?.saldo_aulas ?? 1) > 0 ? 'text-primary' : 'text-ios-red'}
        />
      </div>

      {/* Plano ativo */}
      {planoAtivo ? (
        <div className="bg-white rounded-2xl border border-ios-border p-5 mb-5">
          <div className="flex justify-between items-start mb-3">
            <div>
              <span className="text-[10px] font-black text-primary uppercase tracking-widest">Plano ativo</span>
              <h2 className="text-lg font-black text-gray-900 mt-0.5">{planoAtivo.titulo}</h2>
            </div>
          ...
          <Link
            to="/treinos"
            className="mt-4 flex items-center justify-between p-3 bg-ios-gray rounded-xl active:opacity-70 transition-opacity"
          >
            <span className="text-sm font-bold text-gray-800">Ver exercícios</span>
        ...
        <div className="bg-white rounded-2xl border border-ios-border p-5 mb-5 text-center">
      ...
      {/* Status do aluno */}
      {aluno?.objetivo && (
        <div className="bg-white rounded-2xl border border-ios-border p-4">
          <p className="text-[10px] font-black text-ios-text-secondary uppercase tracking-widest mb-1">Seu objetivo</p>
          <p className="text-sm font-medium text-gray-800">{aluno.objetivo}</p>
        </div>
      )}
    </div>
  );
